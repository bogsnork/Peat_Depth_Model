---
title: "Weardale workthrough with geoR"
...
```{r}
  #Import observations----
data_nogpr.shp <- sf::st_read("../data/weardale/upper_weardale_noGPR.shp")
#data_gpr.shp <- sf::st_read("../data/weardale/upper_weardale_GPRselected.shp")

data_nogpr.shp$GPR <- 0
#data_gpr.shp$GPR <- 1
data.shp <- rbind(data_nogpr.shp)#, data_gpr.shp)
...
data.sp <- sp::SpatialPointsDataFrame(data = data, coords = coords, proj4string = CRS(proj4))
  ...
  #import predictors ----
elev <- raster("../data/weardale/weardale_terrain.tif"); plot(elev)
slope <- resample(raster("../data/weardale/weardale_slp.tif"), elev); plot(slope)


  #create raster stack
predictors <- stack(elev, slope)
names(predictors) <- c("elev", "slope")
plot(predictors)
...
ggplot(mapdata, aes(x = elev, y = peat_depth)) +
  geom_point(aes(colour = peat_depth))+
  scale_color_distiller(palette = "Reds", direction = 1)+
  labs(title = "Peat depth by elevation")

ggplot(mapdata, aes(x = slope, y = peat_depth)) +
  geom_point(aes(colour = peat_depth))+
  scale_color_distiller(palette = "Reds", direction = 1)+
  labs(title = "Peat depth by slope")
...
```{r}
sp_depth <- as.geodata(obj=dat, coords.col=c("x", "y"), data.col="sqrt_peat_depth", covar.col = c("elev", "slope"))
...
(model.geo.exp <- likfit(geodata=sp_depth, trend = ~sp_depth$covariate$elev+sp_depth$covariate$slope,
                    ini.cov.pars=c(4, 100), fix.nugget=FALSE,
...
# Undertake the 10-fold cross validation
   for (i in 1:10) {
    ...
    test.sp <-
      as.geodata(obj=dat.fit, coords.col=c("x", "y"), data.col="sqrt_peat_depth", covar.col = c("elev", "slope"))
...
resid.sp <- as.geodata(obj=dat.fit, coords.col=c("x", "y"), data.col="sqrt_peat_depth", covar.col = c("elev", "slope"))
